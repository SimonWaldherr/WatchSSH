...
var standardToolNames = []string{
	...
	"curl", "docker", "ip", "journalctl", "nc", "openssl", "pgrep", "ping", "ss", "systemctl",
}
...
func discoverStandardTools(ctx context.Context, r runner) map[string]bool {
	const command = "for t in awk cat date df du find grep head hostname ps sed sort stat tail timeout uname uptime who curl docker ip journalctl nc openssl pgrep ping ss systemctl; do command -v \"$t\" >/dev/null 2>&1 && printf '%s\\n' \"$t\"; done"
